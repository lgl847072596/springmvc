...
<title>历史应用列表</title>
<style type="text/css">
   table tr td{text-align:center; line-height:24px;}
</style>
</head>
<body>
...
<div style="text-align:center">
 ...
 <h3><c:if test="${platform =='android'}">Android 历史应用列表</c:if><c:if test="${platform =='ios'}">IOS 历史应用列表</c:if></h3>
 ...
 <tr>
  ...
  <c:forEach var="apk" items="${appList}" varStatus="status">
    <tr>
 		...
 		<td><a href="<%=basePath%>app/public/download/app.action?tb=old&packageName=${apk.packageName}&versionCode=${apk.versionCode}&platform=${apk.platform}&url=${apk.url}">下载</a></td>
 	</tr>
  </c:forEach>
